...
echo "starting sync attempt at `date`"

ip addr show dev ${netifname} > /dev/null 2>&1
if [ $? -eq 0 ]; then
  # interface found
  if [ ${verbose} == 'true' ]; then
    echo "interface ${netifname} found"
  fi
  ping -c 1 ${rm2addr} > /dev/null 2>&1
  if [ $? -eq 0 ]; then
    # remarkable2 is reachable
    if [ ${verbose} == 'true' ]; then
      echo "remarkable reachable on ${rm2addr}"
    fi
    ...
    if [ ! -d ${localbudir} ]; then
      ...
      if [ $? -ne 0 ]; then
        echo "error: local backup directory ${localbudir} does not exist and cannot be created"
	exit 1
      fi
    fi

    # run rsync backup
    rsync -avz --progress --delete ${rm2user}@${rm2addr}:/home/root/.local/share/remarkable/xochitl/ ~/${localbudir}/

  else
    # interface is up but rm2 not reachable
    echo "remarkable not reachable on ${rm2addr}"
    exit 1
  fi
else
  # interface does not exist
  echo "interface ${netifname} not found"
  exit 1
fi
